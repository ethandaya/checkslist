...
contract InfiniteCheckRenderer is IMetadataRenderer {
    ...
    function buildSVG() public view returns (string memory) {
        ...
        for (uint256 i = balanceOf; i < 80; i++) {
            checks = string(abi.encodePacked(checks, checkForIndex(uint8(i), false, user)));
        }
        string memory encoded = Base64.encode(abi.encodePacked(
    ...
    function checkForIndex(uint8 index, bool completed, address user) public view returns (string memory) {
        ...
        string memory color = completed ? string(abi.encodePacked(gradient[colorIndex])) : "#191919";
